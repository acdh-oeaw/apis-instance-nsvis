...
class AnnotationTable(GenericTable):
    issue = tables.TemplateColumn(
        template_code="<a href={{ record.get_absolute_url }}>{{ record.magazine_page.issue }}</a>"
    )
    labelstudio = tables.TemplateColumn(
        template_name="columns/labelstudio_link.html", exclude_from_export=True
    )
    ...
    clip = tables.TemplateColumn(
        template_code="<img class='thumbnail' src='{{ record.clip }}' loading='lazy' onerror='imgError(this);'>",
        exclude_from_export=True,
    )
    author = tables.TemplateColumn(template_name="columns/annotation_author.html")
    photographers = tables.TemplateColumn(
        template_name="columns/annotation_photographers.html"
    )
    ranking = tables.TemplateColumn(
        template_code='<abbr title="{{ record.ranking }}">{{ record.ranking|floatformat:-3 }}</abbr>'
    )
...
class TimespanTable(RelationsListTable):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "/update/" not in kwargs["request"].path:
            self._meta.exclude = RelationsListTable.Meta.exclude + ("actions",)
        else:
            self._meta.exclude = RelationsListTable.Meta.exclude
...
class MagazineIssueTable(GenericTable):
    pages = tables.TemplateColumn(
        template_code="{{ record.magazinepage_set.count }}", orderable=False
    )
